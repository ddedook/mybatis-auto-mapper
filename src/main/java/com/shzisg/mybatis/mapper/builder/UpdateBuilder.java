...
public class UpdateBuilder implements SqlBuilder {
    ...
    private String buildUpdate(boolean needCheck, Map.Entry<String, String> entry, Class<?> type, Class<? extends TypeHandler> typeHandler) {
        StringBuilder builder = new StringBuilder();
        if (needCheck) {
            builder.append("<if test=\"").append(entry.getKey()).append(" != null\">");
        }
        builder.append(entry.getValue())
          .append("=")
          .append(MapperUtils.buildTypeValue(entry.getKey(), type, "", typeHandler))
          .append(",");
        if (needCheck) {
            builder.append("</if>");
        }
